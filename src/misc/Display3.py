...
class Display(object):
	...
	# FIXME: idea: currently we have a monochrome display (black/white). When using ASCII-art we could implement
	#        a grayscale display! :-)
	...
	def update(self):
		...
		for y in row_idx_list:
			if self._dirty_rows[y]:
				...
				for x in range(Display.SCREEN_WIDTH):
					if self._test_bit(x, y):
						chars_list.append(Display.PIXEL_ON)
					else:
						chars_list.append(Display.PIXEL_OFF)
