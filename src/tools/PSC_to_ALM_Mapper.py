...
class ALM_datapoint(object):
	...
	def _collect_Screen(self):
		...
		responses = self._dms_ws.dp_get(path='',
		                                query=dms.Query(regExPath="^(?!BMO).*:ALM:Screen(:[\w]+)?",
		                                                isType="string",
		                                                maxDepth=-1))
		exclusion = ('System', 'GE')
		for respget in responses:
			...
			if not respget.path.startswith(exclusion):
				...
				if respget.path.endswith(':ALM:Screen'):
					...
					logger.warn('ALM_datapoint._collect_Screen(): found unexpected DMS key "' + respget.path + '"... Perhaps new feature in ProMoS NT(R)?')
	...
	def export_into_backupfile(self, project_path):
		...
		with open(name=export_fullpath , mode='w') as f:
			for key, value in self._alm_screen_allkeys_dict.iteritems():
